...
SET @targetargs="test -f netcoreapp1.1 -c Release %@folder%\tests\Services.Web.Api.Integration.Tests"  
SET @filter="+[*]TravelHub.Checkout.* -[*.Tests]* -[*]*.RouteConfig -[Castle.Core]* -[Moq]* -[FluentAssertions*]* -[FluentValidation]* -[xunit.*]*" 
...
%@opencover_exe% ^
 ...
 -skipautoprops ^
 -mergeoutput ^
 -searchdirs:%@folder%\tests\Services.Web.Api.Integration.Tests\bin\Release\netcoreapp2.0 ^
 -hideskipped:All
